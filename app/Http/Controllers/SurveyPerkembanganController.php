...
class SurveyPerkembanganController extends Controller
{
    ...
    {
        $this->middleware('permission:Survey Perkembangan Wilayah', ['except' => ['surveyKelurahan']]);
    }
    ...
    public function getDataSurvey($id_user, $kelurahan, Request $request)
    {
        if ($kelurahan == "*") {
            $data = SurveyPerkembangan::with('image')->where('id_user', $id_user)->orderBy('id_baru', 'DESC')->get();
        } else {
            $data = SurveyPerkembangan::with('image')->where('id_user', $id_user)->where('kelurahan', $kelurahan)->orderBy('id_baru', 'DESC')->get();
        }
        return $data;
    }

    public function getListKelurahanSurvey()
    {
        $data = SurveyPerkembangan::select('kelurahan')->where('id_user', Auth::user()->id)->distinct()->get();
        return $data;
    }
