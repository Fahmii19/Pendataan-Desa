var url = `https://jakarta.pintoinvest.com:3443`;
...
map.on("style.load", function () {
    ...
    map.on(clickEvent, function (e) {
        ...
        // localStorage.setItem("kordinat", JSON.stringify(e));
        let data;
...
map.on(clickEvent, "zoning_fill", function (e) {
    var dt = e.features[0].properties;
    // map.off("idle", "zoning_fill", zoning_loaded);
    // console.log(dt);
    ...
    $.post(`${APP_URL}/save_zoning`, { zoning: dt }).done(() => {
        condition.zoning = true;
        enable_print();
    });
    var gsb = `
...
function addSourceLayer(item) {
    ...
    //     map.fire("click", valueClickEvent);
    // }, 3000);
}

map.on("idle", "zoning_fill", zoning_loaded);

function zoning_loaded(e) {
...
//         if (map.getLayoutProperty("zoning_fill", "visibility") == "none") {
//             console.log("loaded");
//             break;
//         }
//         console.log("loading");
                        ...
                        <div class="row">
                            <div class="col-4">
                                <img width="100px" height="90px" style="object-fit: cover; border-radius:15px"
                                    src="https://jakarta.pintoinvest.com/rent/${
...
$(document).on("click", ".wilayah-select", function (e) {
    $(".wm-search__dropdown").fadeOut();
    $("#btn-titik").show();
    ...
    // geocoder.query(coor);
    searchLocation(coor.split(",")[0], coor.split(",")[1]);
...
function getDataSurvey(id_user, kelurahan = "*") {
    $.ajax({
        ...
        success: (e) => {
            ...
            if (data !== null) {
                data.forEach((item, index) => {
                    ...
                    <div>
                        <div class="p-0">
                            <a class="text-dark font-weight-bold" data-toggle="collapse" href="#data${index}${item.properties.kbli.toString()}"
                                aria-expanded="false" aria-controls="data${index}${item.properties.kbli.toString()}">
                                <span class="collapsed"><i class="fa fa-plus"></i></span>
                                <span class="expanded"><i class="fa fa-minus"></i></span>
                                ${item.properties.kbli.toString()}
                            </a>
                        </div>
                        <div id="data${index}${item.properties.kbli.toString()}" class="collapse">
                            <div class="card-body value-collapse">
                                <span>Jenis Usaha : ${
                                    item.properties.jenis_usaha_kegiatan
                                }</span><br>
                                <span>Skala Besaran Amdal : ${
                                    item.properties.skala_besaran_amdal == ""
                                        ? "-"
                                        : item.properties.skala_besaran_amdal
                                }</span><br>
                                <span>Skala Besaran UKL UPL : ${
                                    item.properties.skala_besaran_ukl_upl == ""
                                        ? "-"
                                        : item.properties.skala_besaran_ukl_upl
                                }</span><br>
                                <span>Skala Besaran SPPL : ${
                                    item.properties.skala_besaran_sppl == ""
                                        ? "-"
                                        : item.properties.skala_besaran_sppl
                                }</span><br>
                                <span>Kategori Amdal UKL UPL : ${
                                    item.properties.kategori_amdal_ukl_upl == ""
                                        ? "-"
                                        : item.properties.kategori_amdal_ukl_upl
                                }</span>
                            </div>
...
const filterLayer = () => {
    ...
    $("#izin_lingkungan").click(function () {
        getDataIzin();
        $(this).css("background", "orange");
...
$("#checkboxKetentuan").change(() => {
    if ($("#checkboxKetentuan").prop("checked") == true) {
        // if ($("#selectPSL").val() == "Pilih...") {
        //     $("#pesanGagalPrintKBLI")
        //         .find("span")
        //         .text("Anda Harus Memilih Kegiatan ITBX.");
        //     $("#pesanGagalPrintKBLI").show();
        //     setTimeout(() => {
        //         $("#pesanGagalPrintKBLI").hide();
        //     }, 3000);
        //     $("#checkboxKetentuan").trigger("click");
        // }
...
//Form Kajian Lingkungan
$("#formKajianLingkungan").on("submit", (e) => {
    e.preventDefault();
    let judul = $("#judulKajian").val();
    let opini = $("#opiniKajian").val();
    $.ajax({
        url: `${APP_URL}/saveKajian`,
        method: "POST",
        beforeSend: () => {
            $("#btnKajianLingkungan").hide();
            $("#prosesKajian").show();
        },
        data: {
            judul: judul,
        ...
        success: (response) => {
            getDataKajian();
            $("#prosesKajian").hide();
            $("#btnKajianLingkungan").show();
            $("#messageSuccessKajian").find("span").text(response.success);
            $("#messageSuccessKajian").show();
            $("#judulKajian").val("");
            $("#opiniKajian").val("");
            setTimeout(() => {
                $("#messageSuccessKajian").hide();
            }, 3000);
        },
        error: (err) => {
            $("#prosesKajian").hide();
            $("#btnKajianLingkungan").show();
            $("#messageFailedKajian").find("span").text(err.responseJSON.error);
            $("#messageFailedKajian").show();
            $("#judulKajian").val("");
            $("#opiniKajian").val("");
            setTimeout(() => {
                $("#messageFailedKajian").hide();
            }, 3000);
        },
    });
});
