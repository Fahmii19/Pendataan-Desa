...
map.on("style.load", () => {
    map.addSource("titik-survey-rekap", {
        ...
        data: `${APP_URL}/dashboard/titik-rekap-survey`,
    });
...
map.on("mouseenter", "titik-survey-rekap", (e) => {
    ...
    $.ajax({
        url: `/v1/dashboard/peta-survey/${id}`,
        method: "GET",
        dataType: "json",
        success: (e) => {
            ...
            var datagambar = "";
            if (array.length == 0) {
                datagambar += `<img class="img_parents" style="width:490px !important;height: 300px; object-fit: cover;" src="https://jakarta.pintoinvest.com/v1/survey/not_image.png" alt="First slide">`;
            } else {
                array.forEach((dataimage) => {
                    datagambar += `<img src="https://jakarta.pintoinvest.com/v1/survey/${dataimage.name}" class="card-img-top" style="width: 100%;height: 100px;object-fit: cover;">`;
                });
            }
